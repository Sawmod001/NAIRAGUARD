...
/**
 * Organization helpers — NG-ORG-01 / NG-103
 * Tenant boundary per docs/05_DATA_MODEL.md + 02-AUTH-ORG-DEMO.md
 * Ensures every user has a personal organization (idempotent).
 * NG-ORG-01: adds slug/type/country + OWNER/MEMBER enum, no currency selector.
 */
...
export async function generateOrganizationSlug(name: string): Promise<string> {
  ...
  // Try base, then base-2, base-3 ... with DB uniqueness check (fast for low cardinality)
  for (let i = 0; i < 10; i++) {
    const candidate = i === 0 ? base : `${base}-${i + 1}`;
    const existing = await prisma.organization.findUnique({ where: { slug: candidate } });
    if (!existing) return candidate;
  }
  // Fallback to base + random suffix
