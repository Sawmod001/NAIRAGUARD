...
export function AppShell({ workspaceName, scenario, accountLabel, children }: { workspaceName: string; scenario: string; accountLabel?: string; children: React.ReactNode }) {
  const [open, setOpen] = useState(false);
  return (
    <div className="min-h-screen bg-[#0E0E0F] text-white">
      {/* Desktop sidebar */}
      <div className="hidden md:fixed md:inset-y-0 md:flex md:w-[260px]">
        <Sidebar workspaceName={workspaceName} scenario={scenario} accountLabel={accountLabel} />
      </div>
      ...
      {open && (
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          <div className="absolute left-0 top-0 h-full w-[260px] bg-[#0E0E0F]">
            <Sidebar workspaceName={workspaceName} scenario={scenario} accountLabel={accountLabel} onNavigate={() => setOpen(false)} />
          </div>
        </div>
      )}
